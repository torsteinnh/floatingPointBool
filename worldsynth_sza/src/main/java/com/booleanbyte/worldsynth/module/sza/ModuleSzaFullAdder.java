...
public class ModuleSzaFullAdder extends AbstractModule {
	...
	public AbstractDatatype buildModule(Map<String, AbstractDatatype> inputs, OutputRequest request) {
		...
		if(inputs.get("a") == null || inputs.get("b") == null || inputs.get("c") == null) {
			return null;
		}
		
		String a = ((DatatypeSza) inputs.get("a")).plastique;
		String b = ((DatatypeSza) inputs.get("b")).plastique;
		String c = ((DatatypeSza) inputs.get("c")).plastique;
		
		if(request.output == getOutput("so")) {
			//a*b*c
			requestData.plastique = "(" + a + "*" + b + "*" + c + ")";
		}
		else {
			//(a+b)*(a+c)*(b+c)
			requestData.plastique = "((" + a + "+" + b + ")*(" + a + "*" + b + "+" + c + "))";
		}
	...
	public Map<String, InputRequest> getInputRequests(OutputRequest outputRequest) {
		...
		inputRequests.put("c", new InputRequest(getInput(2), outputRequest.data));
	...
	public ModuleInput[] registerInputs() {
		ModuleInput[] i = {
				...
				new ModuleInput(new DatatypeSza(), "c")
				};
		return i;
	}
	...
	public ModuleOutput[] registerOutputs() {
		ModuleOutput[] o = {
				new ModuleOutput(new DatatypeSza(), "so"),
				new ModuleOutput(new DatatypeSza(), "co")
				};
		return o;
	}
